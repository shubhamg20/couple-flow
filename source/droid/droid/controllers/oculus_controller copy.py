...
class VRPolicy:
    ...
    def _calculate_action(self, state_dict, include_info=False):
        ...
        # Read Observation
        robot_pos = np.array(state_dict["cartesian_position"][:3]) #xyz
        robot_quat = state_dict["cartesian_position"][3:] 
        robot_euler = R.from_quat([robot_quat[1],robot_quat[2],robot_quat[3],robot_quat[0]]).as_euler('xyz', degrees=False)
        ...
        print("VR:", target_pos_offset*1000)
        target_pos_offset = np.array([
                -target_pos_offset[2],  
                -target_pos_offset[0], 
                target_pos_offset[1]    
        ])
