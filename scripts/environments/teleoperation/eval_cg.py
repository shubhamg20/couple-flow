...
from flow_policy.make_networks import instantiate_flow_cfg_artifacts, instantiate_classifier_artifacts
...
args_cli.checkpoint_cf = "source/serl-flow/chkpts/" + task_name_cf + "/epoch_" + epoch_num_cf +".pt"
...
def load_model_and_config(checkpoint_path, cf_chkpt_path, device='cuda'):
    """
    Load trained model and configuration from checkpoint.
    """
    print(f"Loading checkpoint from: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    checkpoint_cf = torch.load(cf_chkpt_path, map_location=device, weights_only=False)
    
    # Extract config from checkpoint
    cfg = checkpoint['config']
    cfg_cf = checkpoint_cf['config']
    print(f"Loaded config from checkpoint")
    
    # Initialize flow matching artifacts (model only)
    nets, device = instantiate_flow_cfg_artifacts(cfg, model_only=True)
    nets_cf, device = instantiate_classifier_artifacts(cfg_cf, model_only=True)
    ...
    try:
        ...
        state_dict = checkpoint_cf['state_dict']
        flow_state_dict = {}
        
        for key, value in state_dict.items():
            if key.startswith('classifier_net.'):
                flow_state_dict[key.replace('classifier_net.', '')] = value
        # Load weights individually
        if len(flow_state_dict) > 0:
            print(f"Loading classifier_net weights ({len(flow_state_dict)} keys)")
            nets_cf['classifier_net'].load_state_dict(flow_state_dict)
    ...
    return nets, nets_cf, val_dataset, cfg, device, norm_stats
...
def run_diffusion_policy(env, dataset, episode_idx, nets, nets_cf, norm_stats, cfg, num_steps, max_episode_length, device, 
    ...
    with torch.no_grad():
        ...
        for step_idx in tqdm(range(int(len(gt_states)))):
            ...
            class_label = dataset.normalized_train_data['class_labels'][start_idx][0]
            # import pdb; pdb.set_trace()
            ...
            for fm_step in range(int((.0)*num_steps), num_steps):
                ...
                v_uncond = nets['flow_net'](
                    x, t_batch, global_cond=obs_cond
                )
                cf_cond = torch.cat([obs_cond, human_context.unsqueeze(0)], dim=-1)
                s = 2.0
                vt = v_uncond - s*nets_cf['classifier_net'].get_class_gradients(
                    x, t_batch, global_cond=cf_cond, target_class=class_label
                )
                x = x + dt * vt
...
def main():
    ...
    # Load model
    try:
        nets, nets_cf, val_dataset, cfg, device, norm_stats = load_model_and_config(args_cli.checkpoint, args_cli.checkpoint_cf)
    except Exception as e:
