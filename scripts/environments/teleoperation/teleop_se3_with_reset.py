...
class OculusTeleop:
    ...
    def _generate_franka_command(self) -> np.ndarray:
        ...
        # Get the robot base rotation
        robot_base_idx = self.env.scene["robot"].data.body_names.index("panda_link0")  # or root body
        robot_base_quat = self.env.scene["robot"].data.body_quat_w[0, robot_base_idx][[1, 2, 3, 0]].cpu().detach().numpy()  # [w,x,y,z,]
        robot_base_rot = R.from_quat(robot_base_quat)
        robot_base_rot_inv = robot_base_rot.inv()

        eef_quat = eef_quat[[1, 2, 3, 0]].cpu().detach().numpy()  # Convert [w,x,y,z] to [x,y,z,w]
        ...
        cartesian_position = np.concatenate([eef_pos_local, eef_euler])  # [x, y, z, roll, pitch, yaw] - 6 elements
...
def full_environment_reset(env, args_cli, current_joint_pos):
    ...
    """
    # Reset the environment (resets object positions and environment state)
    env.reset()
    ...
    if args_cli.robot == "franka":
        ...
        # Reset root state (robot base position/orientation)
        ...
        # Set joint positions to defaults
    ...
    print("Full environment reset complete - robot, objects, and environment cleared")
...
def main() -> None:
    ...
    # print(f"Joint targets initialized to current positions")
    # print(f"Joint 1 position: {env.scene['robot'].data.joint_pos[0, 0].item():.4f}")
    # print(f"Joint 1 target: {env.scene['robot'].data.joint_pos_target[0, 0].item():.4f}")
    ...
    while simulation_app.is_running():
        try:
            ...
            with torch.inference_mode():
                ...
                if teleop_interface.controller.get_info()["success"]:  # A button pressed
                ...
                if args_cli.robot == "franka":
                    ...
                    #         cv2.circle(img_bgr, (u, v), 2, (0, 0, 255), -1)
                    #     # else:
