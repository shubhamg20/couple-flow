# Usage:
# python /workspace/isaaclab/scripts/environments/teleoperation/pkl_viewer.py --robot franka --task_name sushi --output_video
# python /workspace/isaaclab/scripts/environments/teleoperation/pkl_viewer.py --robot franka --task_name sushi --output_video
# python /workspace/isaaclab/scripts/environments/teleoperation/pkl_viewer.py --robot franka --task_name sushi --show_preview
# python /workspace/isaaclab/scripts/environments/teleoperation/pkl_viewer.py --robot franka --task_name sushi --episode 0  # Process single episode

"""Load and inspect pkl episode files, extract camera frames into video with EE trajectory overlay and gripper state markers."""
...
parser.add_argument("--episode", type=int, default=None, help="Episode number to inspect (if not specified, processes all episodes)")
...
task_dir = Path(f"source/recorded_runs/{args.robot}_toy/{args.task_name}")
output_dir = Path(f"source/recorded_runs/{args.robot}_toy/{args.task_name}/videos")

if args.episode is not None:
    # Process single episode
    episode_files = [task_dir / f"episode{args.episode}.pkl"]
else:
    # Process all episodes
    episode_files = sorted(task_dir.glob("episode*.pkl"))

if not episode_files:
    print(f"Error: No episode files found in {task_dir}")
    exit(1)

print(f"\nFound {len(episode_files)} episode(s) to process")
...
def detect_gripper_events(trajectory):
    """Detect gripper state changes and return list of events."""
    events = []
    prev_gripper = None
    
    for i, step in enumerate(trajectory):
        if 'franka_eef' in step and 'gripper' in step['franka_eef']:
            current_gripper = step['franka_eef']['gripper']
            print(current_gripper, step['action'])
            if current_gripper < .031:
                current_gripper = 1  
            else:
                current_gripper = 0
            if prev_gripper is not None:
                # Detect 0 -> 1 (closing)
                if prev_gripper == 0 and current_gripper == 1:
                    events.append({'frame': i, 'type': 'close', 'text': 'GRIPPER CLOSED'})
                # Detect 1 -> 0 (opening)
                elif prev_gripper == 1 and current_gripper == 0:
                    events.append({'frame': i, 'type': 'open', 'text': 'GRIPPER OPENED'})
...
def draw_gripper_state(img, current_frame, gripper_events, frame_window=30):
    """Draw gripper state changes on frame. Shows text for frame_window frames after event."""
    for event in gripper_events:
        frames_since_event = current_frame - event['frame']
        
        # Show text for frame_window frames after the event
        if 0 <= frames_since_event < frame_window:
            # Calculate fade effect
            alpha = 1.0 - (frames_since_event / frame_window)
            ...
            img = cv2.addWeighted(overlay, 0.7 * alpha, img, 1 - 0.7 * alpha, 0)
            
            # Draw text
            color_with_alpha = tuple(int(c * alpha) for c in color)
            cv2.putText(img, text, (x, y), font, font_scale, color_with_alpha, thickness)
    
    return img


# Process each episode
for pkl_file in episode_files:
    ...
    # print(f"\n{'='*60}")
    # print(f"Episode Data Inspector: {pkl_file.name}")
    # print(f"{'='*60}\n")
    ...
    # print(f"\n{'='*60}")
    # print(f"Gripper Events Detected: {len(gripper_events)}")
    # print(f"{'='*60}")
    # for event in gripper_events:
    #     print(f"  Frame {event['frame']:4d}: {event['text']}")
    ...
    if has_camera_data:
        ...
        print(f"\n  - Camera data: Not available (overlay disabled)")
        ...
        if img_array is not None:
            ...
            if args.output_video or args.show_preview:
                ...
                for i, step in enumerate(episode_data['trajectory']):
                    if 'image' in step and step['image'] is not None:
                        ...
                        if img_array is not None:
                            ...
                            # Draw gripper state changes
                            img_array = draw_gripper_state(img_array, i, gripper_events)
                ...
                if args.output_video and frames_decoded:
                    ...
                    print(f"Gripper events marked: {len(gripper_events)}")
            ...
            if args.output_video:
                ...
                for i, step in enumerate(episode_data['trajectory']):
                    if step.get('camera_frame') is not None:
                        frame = step['camera_frame'].copy()
                        # Draw gripper state changes
                        frame = draw_gripper_state(frame, i, gripper_events)
                        frames_to_save.append(frame)
                
                if frames_to_save:
                    height, width = frames_to_save[0].shape[:2]
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    writer = cv2.VideoWriter(str(video_file), fourcc, 20, (width, height))
