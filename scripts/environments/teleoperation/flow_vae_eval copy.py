...
from flow_policy.make_networks import instantiate_vae_flow_artifacts
...
task_name = "human_object_traj_cfg"
epoch_num = "600"
actual_action = True
bc_policy = False # true for gaussian, false for couple flow
action_replay = False 
state_replay = False
# latent =  None 
...
def load_model_and_config(checkpoint_path, device='cuda'):
    ...
    nets, ema, optimizers, schedulers, dataloader, val_dataloader, dataset, val_dataset, stats, device = instantiate_vae_flow_artifacts(cfg)
    nets['robot_vae'].load_state_dict(checkpoint['robot_vae'])
    nets['flow_net'].load_state_dict(checkpoint['flow_net'])
    nets.eval()
    norm_stats = stats
        ...
        for step_idx in tqdm(range(n)):
            ...
            if not bc_policy:
                if not torch.all(human_action_chunk == -1):
                    _, x, _ = nets['human_vae'].encode(human_action_chunk.unsqueeze(0), current_obs.unsqueeze(0))  # [1, latent_dim]
                    print("using human action encoding:", x.shape)
                ...
                x = torch.randn(1, 16, device=device).unsqueeze(0)

            # _t = .0
            # x = x*_t + (1-_t)*torch.randn(human_action_chunk.shape[0], action_dim, device=device).unsqueeze(0) 6
            # _t = .0 # 0.2-0.8
            # x = torch.randn(1, cfg.latent_dim, device=device).unsqueeze(0)
            _, l_cond, _ = nets['human_vae'].encode(human_action_chunk.unsqueeze(0), current_obs.unsqueeze(0)) 
            # cond_obs = torch.cat([current_obs, l_cond.flatten()], dim=-1)
            uncond_obs = torch.cat([current_obs, torch.zeros_like(l_cond).flatten()], dim=-1)
            ...
            z = x.squeeze(0)  # [1, latent_dim] -> [latent_dim]
            # Add predicted actions to the queue for their corresponding timesteps
            predicted_chunk = nets['robot_vae'].generate_from_latent(z, current_obs.unsqueeze(0)).squeeze(0)
