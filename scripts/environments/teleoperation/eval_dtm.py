...
parser.add_argument("--num_steps", type=int, default=20, help="Number of integration steps for flow matching")
...
VAL_DATASET_PATH = "source/serl-flow/dataset/train_apple.pkl"
# args_cli.checkpoint = "source/serl-flow/chkpts/single_task.pt"
# args_cli.checkpoint = "source/serl-flow/chkpts/multitask_bc2.pt"
args_cli.checkpoint = "source/serl-flow/chkpts/single_apple_dtm/epoch_2000.pt"
explicit_conditioning = True   # Important variable 🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖🤖
...
def run_diffusion_policy(env, dataset, episode_idx, nets, norm_stats, cfg, num_steps, max_episode_length, device, 
                         pose_marker=None, save_trajectory=False, bc_policy=True):
    ...
    with torch.no_grad():
        ...
        # latent_variable =  human_data["eef_poses"]
        if state_replay: 
        ...
        # for step_idx in tqdm(range(10)):
            ...
            for fm_step in range(num_steps):
                ...
                h_t = nets['flow_net'](x, t_batch, global_cond=obs_cond)
                
                Y_0 = torch.randn(1, x.shape[1], x.shape[-1], device=device)
                num_ode_steps = 20
                Y = Y_0.clone()
                ds = 1.0 / num_ode_steps
                for ode_step in range(num_ode_steps):
                    s = torch.rand(1).to(device)
                    u_s = nets['dtm_head'](h_t, t_batch, Y, s)  # Velocity
                    Y = Y + ds * u_s
                x = x + dt * Y
            action_horizon = 8
...
def main():
    ...
    episode_indices = np.array([i for i in range(30)] )
    # import pdb; pdb.set_trace()
    ...
    for idx, episode_idx in enumerate(episode_indices):
        ...
        print(f"\n{'='*60}")
        print(f"Episode {idx + 1}/{args_cli.num_episodes} (dataset episode {episode_idx})")
        print(f"{'='*60}\n")
