...
if args_cli.enable_pinocchio:
    ...
    from isaaclab_tasks.manager_based.manipulation.pick_place.mdp.terminations import object_touching_tray
...
def extract_trajectory_data(env, args_cli, timestep, action):
    ...
    for obj_name in object_names:
        if obj_name in env.scene.keys():
            ...
            # if obj_name == "apple": print(obj_pos)
...
def main() -> None:
    ...
    # Disable automatic resets when objects touch the tray (we want to detect and print instead)
    if hasattr(env_cfg.terminations, 'sushi_on_tray'):
        env_cfg.terminations.sushi_on_tray = None
    if hasattr(env_cfg.terminations, 'apple_on_tray'):
        env_cfg.terminations.apple_on_tray = None
    if hasattr(env_cfg.terminations, 'mug_on_tray'):
        env_cfg.terminations.mug_on_tray = None
    ...
    while simulation_app.is_running():
        try:
            with torch.inference_mode():
                ...
                # print(env.scene["apple"].data.body_pos_w[0][0].cpu().detach().numpy()[2])
                # print(env.scene["sushi"].data.body_pos_w[0][0].cpu().detach().numpy()[2])
                # print(env.scene["mug"].data.body_pos_w[0][0].cpu().detach().numpy()[2])
                if args_cli.teleop_device.lower() == "oculus":
                ...
                if teleoperation_active:
                    # action[3:7] *= 0.5  # Scale down rotation for finer control
                    action[3:7] *= 0.5  # Scale down rotation for finer control
                ...
                # Check if objects are touching the tray and print messages
                if args_cli.enable_pinocchio and args_cli.robot == "franka":
                    try:
                        ...
                        for obj_name in ["sushi", "apple", "mug"]:
                            if obj_name in env.scene.keys():
                                # Get thresholds for this object
                                thresholds = obj_thresholds.get(obj_name, obj_thresholds["sushi"])
                                
                                # Check if object is touching tray
                                is_touching = object_touching_tray(
                                    env,
                                    object_cfg=SceneEntityCfg(obj_name),
                                    tray_cfg=SceneEntityCfg("tray"),
                                    max_distance_x=thresholds["max_distance_x"],
                                    max_distance_y=thresholds["max_distance_y"],
                                    max_distance_z=thresholds["max_distance_z"],
                                    min_distance_z=thresholds["min_distance_z"],
                                    max_velocity=thresholds["max_velocity"],
                                )
                                
                                # Print message if object is touching tray
                                if is_touching[0].item():
                                    if obj_name == "sushi":
                                        print(f"🍣🍣🍣🍣🍣🍣 {obj_name.upper()} TOUCHED TRAY 🍣🍣🍣🍣🍣🍣")
                                    elif obj_name == "apple":
                                        print(f"🍎🍎🍎🍎🍎🍎 {obj_name.upper()} TOUCHED TRAY 🍎🍎🍎🍎🍎🍎")
                                    elif obj_name == "mug":
                                        print(f"☕☕☕☕☕☕ {obj_name.upper()} TOUCHED TRAY ☕☕☕☕☕☕")
                    except Exception as e:
                        # Silently ignore errors (e.g., if tray doesn't exist or function not available)
                        pass
