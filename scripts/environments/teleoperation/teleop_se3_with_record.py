"""Script to run teleoperation with Isaac Lab and record trajectories."""
...
parser = argparse.ArgumentParser(description="Teleoperation for Isaac Lab environments with optional trajectory recording.")
...
class OculusTeleop:
    ...
    def __str__(self) -> str:
        return f"OculusTeleop(robot={self.robot_type}, pos_sensitivity={self.pos_sensitivity}, rot_sensitivity={self.rot_sensitivity})"
    ...
    for obj_name in object_names:
        try:
            if obj_name in env.scene:
                obj_pos = env.scene[obj_name].data.body_pos_w[0].cpu().detach().numpy()
                step_data["objects"][obj_name] = {"pos": obj_pos.tolist()}
        except:
            pass
...
def main() -> None:
    """Run teleoperation with optional trajectory recording."""
    
    # Setup trajectory recording if enabled
    recording_enabled = args_cli.record_trajectory
    trajectory_data = {"episodes": {}, "episode_initial_objects": {}, "metadata": {}}
    current_episode = 1
    current_trajectory = []
    
    if recording_enabled:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        output_dir = Path("recorded_runs")
        output_dir.mkdir(exist_ok=True)
        output_file = output_dir / f"trajectory_data_{timestamp}.pkl"
        ...
        try:
            with torch.inference_mode():
                ...
                if teleop_interface.controller.get_info()["success"]:
                    if recording_enabled:
                        trajectory_data["episodes"][current_episode] = current_trajectory
                        trajectory_data["episode_initial_objects"][current_episode] = episode_initial_objects
                        trajectory_data["metadata"]["num_episodes"] = current_episode
                        with open(output_file, 'wb') as f:
                            pickle.dump(trajectory_data, f)
                        print(f"\n[RECORDING] Episode {current_episode} COMPLETED and SAVED ({len(current_trajectory)} timesteps)")
                        current_episode += 1
                        current_trajectory = []
                        timestep = 0
                        episode_initial_objects = {}
                ...
                if args_cli.robot == "franka":
                    ...
                    pose_marker.visualize(
                        ...
                        orientations=eef_quat.unsqueeze(0),
                    )
                ...
                if recording_enabled:
                    ...
                    if timestep == 0:
                        object_names = ["cube", "banana", "mug", "apples"]
                        for obj_name in object_names:
                            try:
                                if obj_name in env.scene:
                                    obj_pos = env.scene[obj_name].data.body_pos_w[0].cpu().detach().numpy()
                                    episode_initial_objects[obj_name] = {"pos": obj_pos.tolist()}
                            except:
                                pass
    ...
    if recording_enabled:
        trajectory_data["metadata"]["num_episodes"] = current_episode - 1
        with open(output_file, 'wb') as f:
            pickle.dump(trajectory_data, f)
        print(f"\n[RECORDING] Saving trajectory data to: {output_file}")
        print(f"[RECORDING] Total episodes recorded: {trajectory_data['metadata']['num_episodes']}")
