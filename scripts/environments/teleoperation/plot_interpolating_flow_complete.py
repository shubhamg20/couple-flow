...
def run_flow_matching_for_all_tasks(env, task_indices, nets, cfg, device, interpolation_points_batch, obj_init_positions, output_dir):
    """
    Run flow matching inference for all tasks separately using parallel environments.
    """
    print(f"🚀 Starting flow matching experiments for all tasks...")
    
    all_trajectories_data = []
    
    # Run experiments for each task separately
    for task_name, point_indices in task_indices.items():
        print(f"\n{'='*50}")
        print(f"Starting task: {task_name}")
        print(f"{'='*50}")
        
        task_trajectories = run_flow_matching_for_task(
            env=env,
            task_name=task_name,
            task_point_indices=point_indices,
            nets=nets,
            cfg=cfg,
            device=device,
        ...
        all_trajectories_data.extend(task_trajectories)
        
        print(f"Task {task_name} completed with {len(task_trajectories)} trajectories")
    
    print(f"\n🎉 All tasks completed! Total trajectories: {len(all_trajectories_data)}")
    return all_trajectories_data

def run_flow_matching_for_task(env, task_name, task_point_indices, nets, cfg, device, interpolation_points_batch, obj_init_positions, output_dir):
    """
    Run flow matching inference for a specific task using parallel environments and save trajectory videos.
    """
    print(f"🎯 Running flow matching for task: {task_name} with {len(task_point_indices)} points...")
    # Get config parameters
    pred_horizon = cfg.pred_horizon
    obs_horizon = cfg.obs_horizon
    action_horizon = cfg.action_horizon
    
    # Filter interpolation points for this task
    task_points = [interpolation_points_batch[i] for i in task_point_indices]
    batch_size = len(task_points)
    ...
    # current_obs = get_observation_from_env_id(env)
    with torch.no_grad():
        ...
        # # Stack task-specific interpolation points into a batch
        # x_batch = torch.stack(task_points, dim=0)  # [batch_size, pred_horizon, action_dim]

        # # Flow matching inference
        # num_steps = 50
        # dt = 1.0 / num_steps
        # batch_flow_x_values = []  # Store x values at every 5th timestep for all points
        ...
        #     x_batch = x_batch + dt * vt_batch
            
        #     # Store x values every 5 steps
        #     if (fm_step + 1) % 5 == 0:
        #         batch_flow_x_values.append(x_batch.clone().detach().cpu().numpy())
        
        # # Get final predicted actions for all points
        # predicted_actions_batch = x_batch.detach().cpu().numpy()  # [batch_size, pred_horizon, action_dim]
        
        # Run parallel rollouts using multiple environments
        trajectories_data = run_parallel_rollouts_with_videos(
            env=env,
            task_name=task_name,
            task_point_indices=task_point_indices,
            batch_flow_x_values=None, #batch_flow_x_values,
            obj_init_positions=obj_init_positions,
    ...
    print(f"✅ Task {task_name} completed with {batch_size} points")
    return trajectories_data

def run_parallel_rollouts_with_videos(env, task_name, task_point_indices, batch_flow_x_values, obj_init_positions, output_dir, cfg, nets, device, interpolation_points_batch):
    """
    Run multiple rollouts in parallel using Isaac Sim's parallel environments and save videos.
    """
    batch_size = len(task_point_indices)
    num_envs = env.num_envs
    
    print(f"🎬 Running {batch_size} parallel rollouts for task {task_name} using {num_envs} environments...")
    
    # Setup video recording for each environment
    import cv2
    video_writers = []
    video_paths = []
    
    for i, point_idx in enumerate(task_point_indices):
        video_path = output_dir / f"{task_name}_point_{point_idx:03d}.mp4"
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        fps = 30
        img_size = (480, 480)
        video_writer = cv2.VideoWriter(str(video_path), fourcc, fps, img_size)
        video_writers.append(video_writer)
        video_paths.append(str(video_path))
    
    # Initialize trajectories data
    trajectories_data = []
    for i, point_idx in enumerate(task_point_indices):
        trajectory_data = {
            'flow_x_values': [] if batch_flow_x_values is None else [step_data[i:i+1] for step_data in batch_flow_x_values],
            'task': task_name,
    ...
    try:
        # Reset environment and apply initial object positions for all envs
        
        pred_horizon = cfg.pred_horizon
        
        # Run rollouts for multiple batches if needed
        for batch_start in range(0, batch_size, num_envs):
            batch_end = min(batch_start + num_envs, batch_size)
            current_batch_size = batch_end - batch_start
            
            print(f"Running batch {batch_start//num_envs + 1}/{(batch_size + num_envs - 1)//num_envs} with {current_batch_size} rollouts...")
            env.reset()
            env_ids = torch.arange(num_envs, device=env.device)
            apply_demo_objects(env, obj_init_positions, env_ids)   
            
            # Force deterministic physics settling
            for _ in range(20):  # Run exact number of physics steps for consistent settling
                env.step(torch.zeros((num_envs, 7), device=env.device))
            
            # Execute rollout for this batch
            for step in tqdm.tqdm(range(50)):
                if not simulation_app.is_running():
                    break
                
                # Prepare actions for all environments in this step
                step_actions = []
                
                # Get current observations for all environments in this batch
                current_obs_batch = []
                for env_idx in range(current_batch_size):
                    current_obs = get_observation_from_env_id(env, env_idx)
                    if step < 10:
                        current_obs[6] = 0.04
                    trajectories_data[batch_start + env_idx]['ee_positions'].append(current_obs[:3].detach().cpu().numpy())
                    current_obs_batch.append(current_obs)
                if step == 0:
                    print(current_obs)
                # Create observation histories for all environments
                obs_histories = []
                for env_idx in range(current_batch_size):
                    obs_history = deque(maxlen=cfg.obs_horizon)
                    for _ in range(cfg.obs_horizon):
                        obs_history.append(current_obs_batch[env_idx])
                    obs_histories.append(torch.stack(list(obs_history), dim=0))  # [obs_horizon, state_dim]
                
                # Stack all observation histories for batch conditioning
                obs_stack_batch = torch.stack(obs_histories)  # [current_batch_size, obs_horizon, state_dim]
                obs_cond_batch = obs_stack_batch.flatten(start_dim=1)  # [current_batch_size, obs_horizon * state_dim]
                
                # Get latent points for all environments in this batch
                latent_points_batch = []
                for env_idx in range(current_batch_size):
                    point_idx = task_point_indices[batch_start + env_idx]
                    latent_point = interpolation_points_batch[point_idx]
                    latent_points_batch.append(latent_point)
                
                # Stack latent points for batch processing
                x_batch = torch.stack(latent_points_batch).to(device)  # [current_batch_size, pred_horizon, action_dim]
                # Run batched flow matching for all environments at once
                with torch.no_grad():
                    # Flow matching inference
                    num_steps = 50
                    dt = 1.0 / num_steps
                    ...
                    for fm_step in range(num_steps):
                        t = torch.tensor(fm_step * dt, device=device)
                        t_batch = t.unsqueeze(0).repeat(current_batch_size)  # [current_batch_size]
                        
                        # Get velocity prediction for entire batch
                        vt_batch = nets['flow_net'](x_batch, t_batch, global_cond=obs_cond_batch)
                        x_batch = x_batch + dt * vt_batch
                    
                    # Get actions for this step (first action from predicted sequences)
                    predicted_actions = x_batch[:, 0]  # [current_batch_size, action_dim] - first timestep of predictions
                    
                    # Store actions for each environment
                    for env_idx in range(current_batch_size):
                        action = predicted_actions[env_idx].clone()
                        ...
                        trajectories_data[batch_start + env_idx]['executed_actions'].append(action.detach().cpu().numpy())
                
                # Always create actions tensor with exact environment count
                actions_tensor = torch.zeros(num_envs, 7, device=device)
                
                # Fill in actions for current batch
                for env_idx in range(current_batch_size):
                    actions_tensor[env_idx] = step_actions[env_idx]
                
                env.step(actions_tensor)
                
                # Capture frames for videos (only for active environments)
                for env_idx in range(current_batch_size):
                    try:
                        # Get image from specific environment
                        rgb_image = get_image_from_env_id(env, env_idx)
                        rgb_np = rgb_image.detach().cpu().numpy()
                        
                        # Check image format and convert properly
                        if rgb_np.dtype != np.uint8:
                            # If image is in float format (0-1), convert to uint8
                            if rgb_np.max() <= 1.0:
                        ...
                        if rgb_np.shape[-1] != 3:
                            print(f"Warning: Image has {rgb_np.shape[-1]} channels instead of 3")
                            continue
                        ...
                        # Resize if needed
                        if bgr_image.shape[:2] != (480, 480):
                            bgr_image = cv2.resize(bgr_image, (480, 480))
                        
                        # Add text overlay
                        point_idx = task_point_indices[batch_start + env_idx]
                        cv2.putText(bgr_image, f"Task: {task_name} | Point: {point_idx} | Step: {step+1}/150", 
                                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                        
                        video_writers[batch_start + env_idx].write(bgr_image)
                    except Exception as e:
                        print(f"Warning: Failed to capture frame for env {env_idx}: {e}")
                        print(f"Image shape: {rgb_image.shape if 'rgb_image' in locals() else 'N/A'}")
                        print(f"Image dtype: {rgb_image.dtype if 'rgb_image' in locals() else 'N/A'}")
                        print(f"Image min/max: {rgb_image.min()}/{rgb_image.max() if 'rgb_image' in locals() else 'N/A'}")
                
                # Small delay for visualization
                sleep(0.03)
        
    except Exception as e:
        print(f"❌ Error in parallel rollouts for {task_name}: {e}")
        import traceback
        traceback.print_exc()
...
def get_object_poses_from_env_id(env, env_id):
    """Get object poses from a specific environment ID."""
    object_names = ["apple", "mug", "sushi"]
    object_poses = {}
    
    for obj_name in object_names:
        if obj_name in env.scene.keys():
            obj_pos = env.scene[obj_name].data.body_pos_w[env_id, 0].to(env.device)
            object_poses[obj_name] = obj_pos
    
    return object_poses

def get_observation_from_env_id(env, env_id):
    """Get observation from a specific environment ID with consistent coordinate system."""
    # Get robot state (end-effector pose)
    eef_idx = env.scene["robot"].data.body_names.index("panda_hand")
    eef_pos_w = env.scene["robot"].data.body_pos_w[env_id, eef_idx]
    eef_quat_w = env.scene["robot"].data.body_quat_w[env_id, eef_idx][[1, 2, 3, 0]]
    eef_rpy = torch.tensor(R.from_quat(eef_quat_w.detach().cpu().numpy()).as_euler('xyz'), dtype=torch.float32, device=env.device)

    joint_names = env.scene["robot"].data.joint_names
    joint_positions = env.scene["robot"].data.joint_pos[env_id]
    ...
    # Get object positions relative to environment's world frame
    object_poses = get_object_poses_from_env_id(env, env_id)
    object_positions = torch.cat(list(object_poses.values()), dim=-1)
    
    # Important: Use consistent world frame coordinates (not env-specific offsets)
    # All environments should have same object layout, so coordinates should be consistent
    obs = torch.cat([
        eef_pos_w,
...
def get_image_from_env_id(env, env_id):
    """Get image from camera for a specific environment ID."""
    # Try different camera output formats
    try:
        # First try RGB format
        if "rgb" in env.scene["tiled_camera"].data.output:
            rgb_data = env.scene["tiled_camera"].data.output["rgb"]
            image = rgb_data[env_id].clone()
            
            # Check if image is in expected format
            if len(image.shape) == 3 and image.shape[-1] == 3:  # [H, W, 3]
                return image
            elif len(image.shape) == 3 and image.shape[0] == 3:  # [3, H, W] - need to permute
                return image.permute(1, 2, 0)
            elif len(image.shape) == 4:  # [B, H, W, 3] or [B, 3, H, W]
                if image.shape[-1] == 3:  # [B, H, W, 3]
                    return image[0]
                elif image.shape[1] == 3:  # [B, 3, H, W]
                    return image[0].permute(1, 2, 0)
        
        # Try other potential formats
        for key in env.scene["tiled_camera"].data.output.keys():
            print(f"Available camera output: {key}")
            data = env.scene["tiled_camera"].data.output[key]
            print(f"Shape: {data.shape}, dtype: {data.dtype}")
            
        # Fallback - return first available output
        first_key = list(env.scene["tiled_camera"].data.output.keys())[0]
        fallback_data = env.scene["tiled_camera"].data.output[first_key]
        return fallback_data[env_id].clone() if len(fallback_data.shape) > 3 else fallback_data.clone()
        
    except Exception as e:
        print(f"Error getting image from env {env_id}: {e}")
        # Return dummy black image
        return torch.zeros((480, 480, 3), dtype=torch.float32)
...
def main():
    ...
    try:
        # Use more environments for parallel rollouts
        # Memory usage guidelines for 10GB GPU:
        # - Simple scenes: 16-32 envs
        # - Complex scenes with multiple objects: 8-16 envs 
        # - Very complex scenes: 4-8 envs
        num_parallel_envs = 1  # Optimized for 10GB GPU with pick-place scene
        env_cfg = parse_env_cfg(args_cli.task, device=args_cli.device, num_envs=num_parallel_envs)
...
[[  -0.17652340,   -0.64675903,    0.39525372,   -0.06089334,    0.02141328,   -0.20404476,    0.31062007],
 ...
 [  -0.09692258,   -0.30364560,    0.46026950,   -0.00022953,   -0.05903238,   -0.21096796,    0.40301007]]])
            ...
            # sushi = [7,28,32,37,44,45,48,51,52,54,58,60,64,67,68]
            # apple = [0,1,3,8,9,10,12,14,18,19,21,22,24,25,26,27,29,33,34,35,36,38,39,41,42,43,46,49,50,55,56,57,62,63]
            mug   = [2,4,5,6,11,13,15,16,17,20,23,30,31,40,47,53,59,61,65,66,69]
            sushi = [7,28,32,44,45,51,52,58,64,67]
            apple = [1,3,8,10,18,22,24,27,29,33]
            labels = np.zeros(len(random_points), dtype=int) - 1  # Initialize all to -1 (unknown)
            for i in apple:
                labels[i] = 0
            for i in mug:
                labels[i] = 1
            ...
            # Force deterministic physics settling
            for _ in range(10):  # Run exactly 10 simulation steps
                env.step(torch.zeros((env.num_envs, 7), device=env.device))
            
            # Convert all numpy arrays to torch tensors with proper shape
            ...
            # Run batched flow matching for all latent points at once
            print(f"\nRunning batch flow matching for {len(random_points)} points...")
            trajectories_data = run_flow_matching_for_all_tasks(
                env=env,
            ...
            print(f"\nExperiment {exp_idx + 1} completed with all random points")

            # Plot flow matching timesteps analysis AND PCA/t-SNE analysis
            if trajectories_data and len(trajectories_data) > 0:
                # Check if we have flow evolution data
                has_flow_data = any(traj.get('flow_x_values') and len(traj['flow_x_values']) > 0 
                                  for traj in trajectories_data)
                
                if has_flow_data:
                    plot_path = plot_flow_matching_analysis(random_points, trajectories_data, labels, label_names, colors, exp_idx, output_dir)
                    print(f"Saved flow matching analysis plot: {plot_path}")
                
                # Plot PCA and t-SNE analysis on complete action vectors
                # pca_tsne_results = plot_pca_tsne_analysis(random_points, trajectories_data, labels, label_names, colors, exp_idx, output_dir)
                # print(f"Saved PCA & t-SNE analysis plots with {len(pca_tsne_results)} files")
...
def plot_flow_matching_analysis(random_points, trajectories_data, labels, label_names, colors, exp_idx, output_dir):
    ...
    # Get number of flow timesteps stored (every 5th timestep)
    if trajectories_data and 'flow_x_values' in trajectories_data[0] and len(trajectories_data[0]['flow_x_values']) > 0:
        num_flow_steps = len(trajectories_data[0]['flow_x_values'])
    else:
        num_flow_steps = 10  # Default: 10 timesteps from 50 steps (every 5)
    
    # Dynamically calculate grid size based on number of flow steps
    if num_flow_steps <= 4:
        rows, cols = 2, 2
    elif num_flow_steps <= 6:
        rows, cols = 2, 3
    elif num_flow_steps <= 9:
        rows, cols = 3, 3
    elif num_flow_steps <= 12:
        rows, cols = 3, 4
    elif num_flow_steps <= 16:
        rows, cols = 4, 4
    elif num_flow_steps <= 20:
        rows, cols = 4, 5
    elif num_flow_steps <= 25:
        rows, cols = 5, 5
    else:
        # For more than 25, use a larger grid
        rows = int(np.ceil(np.sqrt(num_flow_steps)))
        cols = int(np.ceil(num_flow_steps / rows))
    
    # Create subplots: one for each flow timestep
    fig, axes_raw = plt.subplots(rows, cols, figsize=(cols*3, rows*2.5))
    
    # Convert to flat list for easier indexing
    if rows == 1 and cols == 1:
        axes_list = [axes_raw]
    elif rows == 1 or cols == 1:
        axes_list = axes_raw.flatten() if hasattr(axes_raw, 'flatten') else [axes_raw]
    else:
        axes_list = axes_raw.flatten()
    
    timestep_names = [f"t={i*5}" for i in range(num_flow_steps)]
    
    for t_idx in range(num_flow_steps):
        ax = axes_list[t_idx]
        ...
        for point_idx, traj in enumerate(trajectories_data):
            if 'flow_x_values' in traj and t_idx < len(traj['flow_x_values']):
                ...
                # Use first timestep, first 3 dimensions for 2D visualization
                if x_values.shape[0] == 1:  # Remove batch dimension
                    x_values = x_values[0]  # Shape: [pred_horizon, action_dim]
                
                # Extract first timestep action vector (7 dimensions)
                # Use first 2 dimensions (X, Y) for 2D plotting
                action_vector = x_values[0]  # Shape: [action_dim=7] - action at first timestep
                x, y = action_vector[0], action_vector[1]  # First two action dimensions
                ...
                if t_idx == 0 and point_idx == np.where(labels == label)[0][0]:
        ...
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_title(f'Flow Step {timestep_names[t_idx]}')
    ...
    for i in range(num_flow_steps, rows * cols):
        if i < len(axes_list):
            axes_list[i].set_visible(False)
    
    plt.suptitle(f'Flow Matching Evolution - Experiment {exp_idx + 1}\nHow Latent Points Evolve Through Flow Timesteps', fontsize=16)
    plt.tight_layout()
    
    # Save the plot
    plot_path = output_dir / f"flow_matching_analysis_exp{exp_idx:02d}.png"
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    # Also create a 3D version with selected timesteps (6 for better visibility)
    step_indices = np.linspace(0, num_flow_steps - 1, 6, dtype=int) if num_flow_steps > 6 else np.arange(num_flow_steps)
    selected_timestep_names = [f"t={i*5}" for i in step_indices]
    ...
    for plot_idx, t_idx in enumerate(step_indices):
        ...
        for point_idx, traj in enumerate(trajectories_data):
            if 'flow_x_values' in traj and t_idx < len(traj['flow_x_values']):
                ...
                # Use first timestep, first 3 dimensions for 3D visualization
                if x_values.shape[0] == 1:  # Remove batch dimension
                    x_values = x_values[0]  # Shape: [pred_horizon, action_dim]
                
                # Extract first timestep action vector (7 dimensions)
                # Use first 3 dimensions (X, Y, Z) for 3D plotting
                action_vector = x_values[0]  # Shape: [action_dim=7] - action at first timestep
                x, y, z = action_vector[0], action_vector[1], action_vector[2]  # First three action dimensions
    ...
    plt.suptitle(f'Flow Matching Evolution 3D - Experiment {exp_idx + 1}\nHow Latent Points Evolve Through Flow Timesteps', fontsize=16)
    plt.tight_layout()
    
    # Save the 3D plot
    plot_3d_path = output_dir / f"flow_matching_analysis_3d_exp{exp_idx:02d}.png"
    plt.savefig(plot_3d_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Saved flow matching analysis plot: {plot_path}")
    print(f"Saved flow matching 3D analysis plot: {plot_3d_path}")
    return plot_path
