...
class EpisodeSaver:
    ...
    def _save_episode_to_disk(self, episode_data: EpisodeData):
        ...
        # Convert images from torch tensors to numpy arrays here
        for step in episode_data.trajectory:
            if 'image' in step and isinstance(step['image'], torch.Tensor):
                step['image'] = step['image'].cpu().numpy()
        ...
        with open(episode_file, 'wb') as f:
            pickle.dump({
                ...
                "initial_objects": episode_data.initial_objects
            }, f)
...
def extract_trajectory_data(env, args_cli, timestep, action):
    ...
    if args_cli.robot == "franka":
        ...
        eef_pos_w = get_right_eef_pos(env).cpu().detach().numpy()
        hand = get_hand_state(env).cpu().detach().numpy().flatten()
        print("Hand state: [{}]".format(", ".join("{:.1f}".format(float(h)) for h in hand)))
        if (hand[5:9] < -0.6).all():
    ...
    # Store image as torch tensor - will be converted to numpy in save thread
    ...
    step_data = {
        ...
        "image": img_tensor,  # Keep as torch tensor for now
        "camera": {
            "intrinsics": K.tolist(),
