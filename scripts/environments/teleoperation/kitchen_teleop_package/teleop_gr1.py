...
class EpisodeSaver:
    ...
    def _save_episode_to_disk(self, episode_data: EpisodeData):
        ...
        ##############################################################################
        ##############################################################################
        ##  DEBUG: Save video next to pkl
        ##############################################################################
        ##############################################################################
        debug_frames = []
        for step_data in episode_data.trajectory:
            if 'image' in step_data and isinstance(step_data['image'], torch.Tensor):
                img = step_data['image'].cpu().numpy()
        ...
        ##############################################################################
        ##  DEBUG: Write video file
        ##############################################################################
        if debug_frames:
            video_file = self.task_dir / f"episode{episode_data.episode_num}.mp4"
...
def main() -> None:
    ...
    if args_cli.task is None:
        if args_cli.robot == "franka":
            ...
            args_cli.task = "Isaac-Kitchen-GR1T2-v0"
        else:
