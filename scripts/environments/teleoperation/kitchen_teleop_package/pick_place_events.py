...
def reset_mug_and_ball(
    ...
    """Reset mug and ball with the same random XY offset so the ball always spawns inside the mug."""
    mug = env.scene[mug_cfg.name]
    ball = env.scene[ball_cfg.name]

    mug_root_states = mug.data.default_root_state[env_ids].clone()
    ball_root_states = ball.data.default_root_state[env_ids].clone()

    range_list = [pose_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=mug.device)

    rand_samples = math_utils.sample_uniform(
        ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=mug.device
    )

    # Apply same XY offset to both mug and ball
    positions_mug = mug_root_states[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3]
    positions_ball = ball_root_states[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3]

    mug.write_root_pose_to_sim(
        torch.cat([positions_mug, mug_root_states[:, 3:7]], dim=-1), env_ids=env_ids
    )
    mug.write_root_velocity_to_sim(torch.zeros((len(env_ids), 6), device=mug.device), env_ids=env_ids)

    ball.write_root_pose_to_sim(
        torch.cat([positions_ball, ball_root_states[:, 3:7]], dim=-1), env_ids=env_ids
    )
    ball.write_root_velocity_to_sim(torch.zeros((len(env_ids), 6), device=ball.device), env_ids=env_ids)
