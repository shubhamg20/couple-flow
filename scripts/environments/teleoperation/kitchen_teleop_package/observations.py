...
def object_obs(
    ...
    banana_pos = env.scene["banana"].data.root_pos_w - env.scene.env_origins
...
def object_obs_single(
    env: ManagerBasedRLEnv,
) -> torch.Tensor:
    """
    Single-object observations for GR1T2 (scene has "object" only, no banana/apple/mug).
    Returns: object pos, object quat, left_eef_to_object, right_eef_to_object.
    """
    body_pos_w = env.scene["robot"].data.body_pos_w
    left_eef_idx = env.scene["robot"].data.body_names.index("left_hand_roll_link")
    right_eef_idx = env.scene["robot"].data.body_names.index("right_hand_roll_link")
    ...
    left_eef_to_obj = obj_pos - left_eef_pos
    right_eef_to_obj = obj_pos - right_eef_pos

    return torch.cat((obj_pos, obj_quat, left_eef_to_obj, right_eef_to_obj), dim=1)
